...
# MoviePy 안전 import (클라우드 환경 예외 처리)
try:
...
st.caption("📱 모바일 기기 완벽 지원: 10만+ 뷰 대본, 쿠팡 파트너스 연동, 마케팅 패키지 생성")
st.markdown("---")
...
with tab1:
    ...
    with col1:
        ...
        start_btn = st.button("🚀 마케팅 패키지 및 텍스트 자동 생성", key="btn_shop")
...
"""
        st.success("🎉 상훈's AI 마케팅 패키지 생성이 완료되었습니다!")
        with col2:
            st.text_area("📋 [쿠팡 파트너스 링크가 포함된 마케팅 문구]", value=upload_text, height=450)
...
with tab2:
    ...
    with col1:
        ...
        start_pl_btn = st.button("🎧 SEO 마케팅 패키지 생성", key="btn_pl")

    with col2:
        st.subheader("📊 미리보기 및 메타패키지")
...
"""
        st.success("🎉 상훈's AI 롱폼 마케팅 패키지 생성이 완료되었습니다!")
        with col2:
            st.text_area("📋 [SEO 상세 메타패키지]", value=seo_package_text, height=450)
